...
- **Remote tmux sessions** across hosts you configure (`tt tmux ls`, attach, `kill`, `clean`, `recover`, `tui`).
- **Code-org workspace scheme** — `~/code/<track>/<domain>/<workspace>.<born>/<repo>` (`ls`, `new-project`).
...
# standalone pipx install
pipx inject geno-tt iterm2
# then enable: iTerm2 ▸ Settings ▸ General ▸ Magic ▸ Enable Python API
```

If it is missing, `tt iterm` prints an install command using the exact Python
environment that runs the active `tt` executable.
