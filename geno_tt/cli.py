...
def _default_vscode_theme(installed_themes: set[str] | None = None) -> str:
    ...
    nothing matched: VS Code ignores an unknown `workbench.colorTheme` and keeps
    the user's current theme, so a wrong guess is inert — whereas refusing to
    write the file at all would leave the workspace unopenable as a workspace.
    """
    installed = (
